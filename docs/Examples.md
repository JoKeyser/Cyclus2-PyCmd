...
The tabular format was reformatted into numbered lists, for easier reading on small screens and terminal applications.

If in doubt, please refer to the original PDF for the official content.
In case of discrepancies, the PDF is authoritative; please report any issues to the authors.

Here is how to interpret a single step in the examples:

- `Command to Cyclus2` → `Reply from Cyclus2` — Comments about the command and reply.
